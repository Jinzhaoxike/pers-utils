 ...
 * @Create : 2019/08/09 10:39
 */
public class MaxLengthValidate extends AbstractValidate {
    @Override
    protected ValidateResult validate(String fieldName, String fieldValue, String name, String validateValue) {
        ...
        builder.setValid(isMatched)
                .setMessage(isMatched ? "匹配成功" : String.format("字段[%s(%s)]长度不正确，最大长度：%d，实际长度：%d", name, fieldName, length, valueLength));
